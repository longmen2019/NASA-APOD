...
def main():
    ...
    if choice == 2:
        ...
        cap = cv2.VideoCapture(0)  # Captures video frames from the webcam using cv2.VideoCapture(0)
        while True:
            ret, image = cap.read()
            if not ret:
        ...
        cv2.destroyAllWindows()  # Closes any open windows used by OpenCV
        ...
        data, bbox, straight_qrcode = detector.detectAndDecode(img)  # Detects and decodes QR codes within the image
