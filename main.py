...
def decoder(image):
    ...
    for obj in barcode:
        ...
        cv2.putText(frame, string, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)  # This line displays the string information (data and type) on the image at the top-left corner of the bounding rectangle ((x, y)) using a specific font and color.
...
if choice == 2:
    ...
    cap = cv2.VideoCapture(0) #The code captures video frames from the webcam using cv2.VideoCapture(0). (0 refers to the default webcam)
    while True: #It enters a loop that continuously captures frames and calls the decoder function on each frame to process and decode any barcodes present
        ret, image = cap.read()
        decoder(frame)
        cv2.imshow("Image", frame) #The processed frames with highlighted barcodes and decoded data are displayed using cv2.imshow
        code = cv2.waitKey(10)
    ...
    img_path = input("Enter Image Path: ") #This line prompts the user to enter the file path of the image containing the QR code
    img = cv2.imread(img_path) #This line uses the cv2.imread function from OpenCV to load the image specified by the img_path into a NumPy array named img
